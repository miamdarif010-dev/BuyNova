...
class _UserProfilePageState extends State<UserProfilePage> {
  ...
  // TODO: à¦¸à¦¬ à¦…à§à¦¯à¦¾à¦¡à¦®à¦¿à¦¨ à¦‡à¦‰à¦œà¦¾à¦°à§‡à¦° à¦¡à¦•à§à¦®à§‡à¦¨à§à¦Ÿà§‡ role: 'admin' à¦¸à§‡à¦Ÿ à¦•à¦°à¦¾à¦° à¦ªà¦°
  // à¦¨à¦¿à¦šà§‡à¦° à¦‡à¦®à§‡à¦‡à¦² à¦«à¦²à¦¬à§à¦¯à¦¾à¦•à¦Ÿà¦¾ à¦®à§à¦›à§‡ à¦¦à¦¿à¦“à¥¤
  bool get _isAdmin {
    return _role == 'admin' ||
  ...
  // =========================================================
  // LIVE USER DATA (à¦…à§à¦¯à¦¾à¦¡à¦®à¦¿à¦¨ approve à¦•à¦°à¦²à§‡ à¦¸à¦™à§à¦—à§‡ à¦¸à¦™à§à¦—à§‡ à¦†à¦ªà¦¡à§‡à¦Ÿ)
  // =========================================================
  ...
  // Pull-to-refresh à¦à¦¬à¦‚ EditProfile à¦¥à§‡à¦•à§‡ à¦«à§‡à¦°à¦¾à¦° à¦ªà¦° à¦¬à§à¦¯à¦¬à¦¹à¦¾à¦° à¦¹à¦¯à¦¼
  ...
  void _applyUserData(Map<String, dynamic>? data) {
    ...
    setState(() {
      ...
      _sellerStatus = (data?['sellerStatus'] ?? '').toString();
      _entrepreneurStatus = (data?['entrepreneurStatus'] ?? '').toString();
  ...
  // à¦¨à¦¤à§à¦¨ à¦ªà§‡à¦œ (à¦†à¦—à§‡ "coming soon" à¦›à¦¿à¦²)
  void _openShopProfile() => _push(const ShopProfilePage());
  void _openShopLocation() => _push(const ShopLocationPage());
  void _openSalesOrders() => _push(const SalesOrdersPage());
  void _openResellerOrders() => _push(const ResellerOrdersPage());
  void _openResellerProfit() => _push(const ResellerProfitPage());
  void _openResellerMessages() => _push(const ResellerMessagesPage());
  void _openSellerVideoRewards() => _push(const SellerVideoRewardsPage());
  void _openReferral() => _push(const ReferralPage());
  ...
  Future<void> _becomeSeller() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Seller request submitted successfully.'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Could not submit seller request: $e')),
        );
      }
    } finally {
      if (mounted) {
    ...
    try {
      await FirebaseFirestore.instance
          ...
          .set(
        {
          ...
          'entrepreneurRequestedAt': FieldValue.serverTimestamp(),
        },
        SetOptions(merge: true),
      );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Reseller request submitted successfully.'),
            behavior: SnackBarBehavior.floating,
          ),
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Could not submit reseller request: $e')),
        );
      }
    } finally {
      if (mounted) {
      ...
      return ClipOval(
        child: Image.network(
          ...
          errorBuilder: (context, error, stackTrace) {
            return const Icon(Icons.person, size: 44, color: Colors.grey);
          },
        ),
      );
    }
  ...
  // =========================================================
  // SHARED CARD HELPER (Card + ListTile)
  // =========================================================
  ...
  static const Widget _chevron = Icon(Icons.chevron_right);
  ...
  Widget _rewardsSummaryCard() {
    return _infoCard(
      ...
      title: 'My Points',
      subtitle: '$_pointsBalance points available',
      fontWeight: FontWeight.w600,
      trailing: _chevron,
  ...
  List<Widget> _sellerSectionItems() {
    if (_isSellerApproved) {
      return [
        ...
        _infoCard(
          ...
          subtitle: 'Manage your BuyNova seller business.',
          fontWeight: FontWeight.bold,
          trailing: _chevron,
    ...
    if (_sellerStatus == 'pending') {
      return [
        ...
        _infoCard(
          ...
          subtitle: 'Your seller request is waiting for admin approval.',
          fontWeight: FontWeight.bold,
          fontSize: 16,
        ),
      ];
    ...
    if (_sellerStatus == 'rejected') {
      return [
        ...
        _infoCard(
          ...
          subtitle: 'You can submit a new seller request.',
          fontWeight: FontWeight.bold,
          fontSize: 16,
          trailing: TextButton(
    ...
    return [
      ...
      _infoCard(
        ...
        subtitle: 'Apply to sell your products on BuyNova.',
        fontWeight: FontWeight.bold,
        fontSize: 16,
        trailing: _chevron,
        onTap: _isSubmitting ? null : _becomeSeller,
      ),
    ];
  }
  ...
  List<Widget> _entrepreneurSectionItems() {
    if (_isEntrepreneurApproved) {
      return [
        ...
        _infoCard(
          ...
          subtitle: 'Manage your BuyNova reseller business.',
          fontWeight: FontWeight.bold,
          fontSize: 16,
          trailing: _chevron,
    ...
    if (_entrepreneurStatus == 'pending') {
      return [
        ...
        _infoCard(
          ...
          subtitle: 'Your reseller request is waiting for admin approval.',
          fontWeight: FontWeight.bold,
          fontSize: 16,
        ),
      ];
    ...
    if (_entrepreneurStatus == 'rejected') {
      return [
        ...
        _infoCard(
          ...
          subtitle: 'You can submit a new reseller request.',
          fontWeight: FontWeight.bold,
          fontSize: 16,
          trailing: TextButton(
            onPressed: _isSubmitting ? null : _becomeEntrepreneur,
            child: const Text('Apply Again'),
          ),
        ),
      ];
    ...
    return [
      ...
      _infoCard(
        ...
        subtitle: 'Apply to become a BuyNova reseller.',
        fontWeight: FontWeight.bold,
        fontSize: 16,
        trailing: _chevron,
        onTap: _isSubmitting ? null : _becomeEntrepreneur,
      ),
    ];
  }
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          children: [
                            ...
                            Text(
                              _name.isNotEmpty ? _name : 'BuyNova User',
                              style: const TextStyle(
                                fontSize: 22,
                                fontWeight: FontWeight.bold,
                              ),
                    ...
                    if (_buyerExpanded) ..._buyerSectionItems(),
                    ...
                    _expandableSectionHeader(
                      ...
                      onTap: () {
                        setState(() {
                          _entrepreneurExpanded = !_entrepreneurExpanded;
                        });
                      },
                    ),

                    if (_entrepreneurExpanded) ..._entrepreneurSectionItems(),
                    ...
                    if (_sellerExpanded) ..._sellerSectionItems(),
